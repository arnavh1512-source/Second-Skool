...
function StudentRoster({ list }: { list: Student[] }) {
  if (list.length === 0) return <div className="text-td-caption text-td-muted py-2.5 px-1">No students here yet</div>
  return (
      ...
      {list.map((s, i) => (
        <div key={s.dbId ?? s.id} className="bg-td-bg border border-td-border rounded-td-md p-[11px] px-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-td-small font-bold text-td-dark truncate">{s.name}</div>
            <div className="text-td-caption text-td-muted truncate">{s.klass}{s.school ? ` · ${s.school}` : ''}</div>
            <div className="text-td-caption text-td-subtle truncate">{s.id}{s.parent ? ` · ${s.parent}` : ''}</div>
          </div>
          <span className="text-td-caption font-bold py-1 px-2.5 rounded-td-lg shrink-0" style={{ color: feeColor(s.feeStatus).c, background: feeColor(s.feeStatus).b }}>{s.feeStatus}</span>
        </div>
      ))}
    </div>
  )
          ...
          {branchesList.map(b => {
            ...
            <div key={b.dbId ?? b.name} className="td-card rounded-td-lg p-4">
              <div className="flex items-center justify-between mb-2.5">
                ...
                {b.main && <span className="text-td-caption font-bold text-td-primary bg-td-tint-blue py-1 px-[9px] rounded-td-lg">Main</span>}
              </div>
              ...
              <div className="flex items-center justify-between">
                <button onClick={() => setOpenBranch(open ? null : b.name)} className="td-plain flex gap-[18px] p-0 cursor-pointer text-left">
                  <div><div className="text-base td-strong">{roster.length}</div><div className="text-td-caption text-td-subtle font-semibold">Students {roster.length > 0 && <span className="text-td-primary">{open ? '▲' : '▼'}</span>}</div></div>
                  <div><div className="text-base td-strong">{b.staff}</div><div className="text-td-caption text-td-subtle font-semibold">Staff</div></div>
                </button>
                {b.dbId && <button onClick={() => deleteBranch(b.dbId!)} className="td-danger text-td-caption font-bold py-2 px-3.5 rounded-td-sm">Remove</button>}
              </div>
            ...
            return (
              <div key={r.dbId ?? r.name} className="td-card rounded-td-md p-[13px] px-[15px]">
                <div className="flex items-center gap-[13px]">
                  <div className="w-10 h-10 rounded-td-sm shrink-0 flex items-center justify-center text-white font-bold text-td-body" style={{ background: av(i) }}>{r.name[0]}</div>
                  {list ? (
                    <button onClick={() => setOpenRow(open ? null : r.name)} className="td-plain flex-1 min-w-0 p-0 cursor-pointer text-left">
                      <div className="text-td-body font-bold text-td-dark truncate">{r.name}</div>
                      <div className="text-td-caption text-td-muted font-semibold">{list.length} student{list.length === 1 ? '' : 's'} {list.length > 0 && <span className="text-td-primary">{open ? '▲' : '▼'}</span>}</div>
                    </button>
                  ) : (
                    <div className="flex-1 text-td-body font-bold text-td-dark truncate">{r.name}</div>
                  )}
                  {r.dbId && <button onClick={() => setConfirm({ id: r.dbId!, name: r.name })} className="td-danger text-td-caption font-bold py-1.5 px-3 rounded-td-sm shrink-0">Remove</button>}
                </div>
