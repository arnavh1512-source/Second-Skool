...
import { useDashboard, initials, feeTag, type Student } from '../store'
    ...
    <div className="flex flex-col gap-2 mt-1">
      {list.map(s => (
        <div key={s.dbId ?? s.id} className="bg-td-bg border border-td-border rounded-td-md p-[11px] px-3 flex items-center gap-3">
          ...
          <span className={`td-tag px-[7px] py-[3px] shrink-0 ${feeTag(s.feeStatus)}`}>{s.feeStatus}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div className="td-screen">
      ...
      {showForm && (
        <div className="td-form-card mb-[18px]">
          <div className="text-td-small td-strong">New branch</div>
                ...
                <div className="flex items-center gap-[13px]">
                  ...
                  {list ? (
                    <button onClick={() => setOpenRow(open ? null : r.name)} className="td-plain flex-1 min-w-0 p-0 cursor-pointer text-left">
                      <div className="text-td-body font-semibold text-td-dark truncate">{r.name}</div>
