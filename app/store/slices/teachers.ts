...
export const createTeachersSlice: Slice<'setNewTeacher' | 'saveTeacher'> = (set, get) => ({
  ...
  saveTeacher: async () => {
    ...
    if (!nt.name.trim()) { get().notify('Enter a name first', 'error'); return }
    if (!nt.qualification.trim()) { get().notify('Enter qualification', 'error'); return }
    if (nt.experience && isNaN(Number(nt.experience))) { get().notify('Experience must be a number', 'error'); return }
    ...
    const { data, error } = await supabase
      ...
      .select().single()
    if (error || !data) { get().notify('Could not add teacher — check your connection', 'error'); return }
