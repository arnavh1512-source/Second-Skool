...
// `recent` marks a phone added this week: the first phone on a code is allowed
// without asking, so a code that leaked before the family used it would
// otherwise sit in the list looking exactly like theirs.
export interface StudentDevice { dbId: string; studentId: string; studentName: string; label: string; allowed: boolean; recent: boolean; when: string; lastSeen: string | null }
